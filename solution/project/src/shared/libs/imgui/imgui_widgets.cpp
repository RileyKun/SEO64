...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x) {
            // draw hamburger icon instead of arrow
            /* seo64 change */
            const float height{ arrow_size / 4.f };
            const float width{ floorf( arrow_size * 0.75f) };
            const float add_x{ floorf(arrow_size * 0.25f) };

            for (int i = 0; i < 3; i++) {
                const float y{ 1.0f + floorf(bb.Min.y + style.FramePadding.y + height * i + height / 2.f) };

                window->DrawList->PathLineTo(ImVec2(add_x + value_x2 + style.FramePadding.x, y));
                window->DrawList->PathLineTo(ImVec2(add_x + value_x2 + width - style.FramePadding.x, y));
                window->DrawList->PathStroke(text_col, 0, 1.0f);
            }
...
/* seo64 change */
bool ImGui::SelectableWithValueOnRight(const char *label, bool selected, const char *value, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (is_visible) {
        ...
        ImVec2 value_size = CalcTextSize(value, NULL, true);
        ImVec2 value_pos = ImVec2(bb.Max.x - value_size.x - style.FramePadding.x, text_min.y);
        RenderTextClipped(value_pos, bb.Max, value, NULL, &value_size, style.SelectableTextAlign, &bb);
    }
